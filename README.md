...
sequenceDiagram
    ...
    participant UIS as 🖥️ UI Server
    participant SS as 🔊 Stream Server
    participant DB as 🐘 PostgreSQL
    participant LS as 💽 Local Storage

    U->>UI: Search for "Song A"
    UI->>N: /search?q=Song A
    N->>UIS: Route to UI Server
    UIS->>DB: SELECT * FROM songs WHERE title LIKE '%Song A%'
    DB->>UIS: Return matching songs
    UIS->>UI: Display search results
    UI->>U: Show song list
    
    U->>UI: Click Play button
    UI->>N: /stream/song_id
    N->>SS: Route to Stream Server
    SS->>LS: Read audio file
    LS->>SS: Stream audio data
    SS->>U: Stream audio to browser
```
...
sequenceDiagram
    ...
    participant YT as ▶️ yt-dlp
    participant LS as 💽 Local Storage
    participant DB as 🐘 PostgreSQL

    U->>UI: Paste YouTube URL
    UI->>UIS: POST /download with URL
    UIS->>R: Create download task
    UIS->>UI: Task queued successfully
    
    Note over CW: Background Processing
    CW->>R: Pick up download task
    CW->>YT: Execute download
    YT->>LS: Save audio file
    CW->>DB: Insert song metadata
    CW->>R: Mark task complete
```
...
<details>
<summary><strong>🛠️ Troubleshooting Commands</strong></summary>
